...
if not exist %FILE% (
    echo APP_MODE=LOCAL > %FILE%
    echo File .env creato con APP_MODE=LOCAL
    exit /b
)

echo Modificando .env in LOCAL...
...
rem Aggiunge la nuova riga APP_MODE=LOCAL
echo APP_MODE=LOCAL>>%TEMP_FILE%

rem Sostituisce il file originale con quello modificato
move /Y %TEMP_FILE% %FILE% >nul

echo Modalità impostata su LOCAL.
